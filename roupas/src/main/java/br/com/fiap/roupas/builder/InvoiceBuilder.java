...
public class InvoiceBuilder {
	...
	private static final Integer TOTAL_COLUMNS = 70;
	...
	public String build() {
		Integer[] sizeColumns = { 5, 7, 14, 10, 5, 14, 15 };
		Integer columnIndex;
		...
		invoiceText.append(StringUtils.leftPad("VL.UNIT.(R$)", sizeColumns[columnIndex++]));
		invoiceText.append(StringUtils.leftPad("VL.TOTAL.(R$)", sizeColumns[columnIndex++]));
		invoiceText.append("\n");
	...
	private BigDecimal getTotalValue() {
		return items.stream().map(ItemVO::getTotalValue).reduce(BigDecimal.ZERO, (v1, v2) -> v1.add(v2));
	}
}
